...
function createVisualizerHTML(value, type) {
    let percentage = 0; let gradientClass = ''; let labels = ['', ''];
    if (type === 'svi') {
        percentage = (value || 0) * 100; gradientClass = 'svi-gradient'; labels = ['Least Vuln.', 'Most Vuln.'];
    } else if (type === 'precip') {
        percentage = Math.min((value || 0) / 100, 1) * 100; gradientClass = 'precip-gradient'; labels = ['0 in', '100+ in'];
    }
...
function setupInteractionListeners() {
    ...
    map.on('click', (e) => {
        ...
        const feature = features[0]; let content = '';
        switch (feature.layer.id) {
            ...
            case 'flood-points-layer':
                ...
                content = `<h3>Flood Event</h3><p>County: <strong>${feature.properties.CZ_NAME_STR}</strong></p><p>Date: <strong>${date}</strong></p>`;
                break;
            case 'svi-layer':
                ...
                if (selectedThemeKey === 'RPL_THEMES_state') {
                    content += `<p>Overall Vulnerability: <strong>${(props.RPL_THEMES_state * 100).toFixed(1)}th percentile</strong></p>${createVisualizerHTML(props.RPL_THEMES_state, 'svi')}<hr style="border: none; height: 1px; background-color: var(--border-color); margin: 15px 0;">`;
                    Object.keys(SVI_DATA).forEach(themeKey => {
                        if (themeKey !== 'RPL_THEMES_state') content += `<p>${SVI_DATA[themeKey].title}: <strong>${(props[themeKey] * 100).toFixed(1)}th percentile</strong></p>`;
                    });
                } else {
                    ...
                    content += `<p>${factorTitle}: <strong>${(props[factorToVisualize] * 100).toFixed(1)}th percentile</strong></p>${createVisualizerHTML(props[factorToVisualize], 'svi')}<hr style="border: none; height: 1px; background-color: var(--border-color); margin: 15px 0;">`;
                    for (const factorKey in SVI_DATA[selectedThemeKey].factors) {
                        content += `<p>${SVI_DATA[selectedThemeKey].factors[factorKey]}: <strong>${(props[factorKey] * 100).toFixed(1)}th percentile</strong></p>`;
                    }
                }
                break;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    for (const key in SVI_DATA) {
        sviThemeSelect.add(new Option(SVI_DATA[key].title, key));
    }
